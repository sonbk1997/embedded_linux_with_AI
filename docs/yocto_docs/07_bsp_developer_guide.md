...
Layer using the bitbake-layers 
tool.
...
through
a web interface at `/`. If you go to that interface,
...
meta-yocto . The `meta-yocto-bsp` layer maintains
...
bsp-guide/bsp:developing a board support package (bsp)
section. For more
...
script (i.e. structure-core-script).
...
> **Note:**
> Ordering and **BBFILE_PRIORITY** for the layers listed in **BBLAYERS**
...
are known as "**container layers <Container Layer>**". An example of
this type of layer is OpenEmbedded's meta-openembedded 
...
> bsp-guide/bsp:example filesystem layout section.
>

#. *Set Up the Build Environment:* Be sure you are set up to use BitBake
   in a shell. See the "dev-manual/start:preparing the build host"
...
#. *Prepare the Source Repositories*:
   ...
   There are two ways of setting up the **Poky** reference distro:
...
#. *Determine the BSP Layer You Want:* The Yocto Project supports many
   ...
   to contain several BSPs. The `Layer Index <>` can help finding
   BSP layers to use.
...
#. *Initialize the Build Environment:* While in the **Source Directory**,
   run the structure-core-script environment setup
   script located in **OpenEmbedded-Core (OE-Core)** to define the OpenEmbedded
...
"bsp-guide/bsp:released bsp requirements" section.
...
the "dev-manual/licenses:maintaining open source license compliance during your product's lifecycle"
section in the Yocto Project Development Tasks Manual.
...
available in the `Yocto Project <>` website. You can
...
within **OpenEmbedded-Core (OE-Core)**.
...
recipe found in **OpenEmbedded-Core (OE-Core)**
at
...
the "kernel-dev/common:creating the append file" section
...
the "bsp-guide/bsp:creating a new bsp layer using the \\`bitbake-layers\`\` script`"
section.

The following illustration and list summarize the BSP creation general
workflow.

![bsp-dev-flow.png](figures/bsp-dev-flow.png)

#. *Set up Your Host Development System to Support Development Using the
   Yocto Project*: See the "dev-manual/start:preparing the build host"
...
#. *Create Your Own BSP Layer Using the bitbake-layers Script:*
   ...
   layers, see the "overview-manual/yp-intro:the yocto project layer model"
   section in the Yocto Project Overview and Concepts Manual. You can also
   reference the "dev-manual/layers:understanding and creating layers"
   section in the Yocto Project Development Tasks Manual. For more
   information on BSP layers, see the "bsp-guide/bsp:bsp layers"
   section.
   ...
   layout. This layout is described in the "bsp-guide/bsp:example filesystem layout"
   ...
   BSP found in the meta-intel  layer.
...
-  The requirements here assume the BSP layer is a well-formed, "legal"
   ...
   "bsp-guide/bsp:bsp layers" section in this manual and the
   "dev-manual/layers:understanding and creating layers"
   section in the Yocto Project Development Tasks Manual.
...
-  *Layer Name:* The BSP must have a layer name that follows the Yocto
   ...
   "bsp-guide/bsp:bsp layers" section.
...
-  *File System Layout:* When possible, use the same directory names in
   ...
   **OpenEmbedded-Core (OE-Core)**.
   ...
   what is found in the **OpenEmbedded-Core (OE-Core)** Git repository.
   In other
   ...
   recipes found in the "/contributor-guide/recipe-style-guide"
...
-  *README File:* You must include a `README` file in the
   ...
   -  The name and contact information for the BSP layer maintainer.
      ...
      /contributor-guide/submit-changes section in the Yocto Project and
...
-  *Use a Yocto Linux Kernel:* Kernel recipes in the BSP should be based
   ...
   `Source Repositories <>` for these kernels.
...
#. Create and place the new `interfaces` configuration file in the
   ...
   The **FILESEXTRAPATHS** variable in the append files extends the search
...
"SOFTWARE" tab on the `Yocto Project Website <>` makes
...
#. *Use the LICENSE_FLAGS Variable to Define the Recipes that Have Commercial or
   ...
   **LICENSE_FLAGS_ACCEPTED**.
   ...
   the **LICENSE_FLAGS_ACCEPTED** variable, the build stops and provides
   you with the list of recipes that you have tried to include in the image
   that need entries in the **LICENSE_FLAGS_ACCEPTED** variable. Once you
   ...
   **LICENSE_FLAGS_ACCEPTED** variable, you can build the encumbered
...
#. *Get a Pre-Built Version of the BSP:* You can get this type of BSP by
   ...
   `Yocto Project website <>`. You can download BSP tarballs
   ...
   **LICENSE_FLAGS_ACCEPTED** to match the encumbered recipes in the
   BSP.
...
-  *Create a Layer Configuration File:* Every layer needs a layer
   ...
   `Source Repositories <>`. To get examples of what you need
...
-  *Create a Kernel Recipe:* Create a kernel recipe in
   ...
   examples. See the "kernel-dev/common:modifying an existing recipe"
...
the "ref-manual/variables:variables glossary" in the Yocto
Project Reference Manual.

-  **PREFERRED_PROVIDER_virtual/xserver <PREFERRED_PROVIDER>**:
   ...
   `meta/recipes-graphics/xorg-xserver` in **OpenEmbedded-Core (OE-Core)**.
...
-  **WKS_FILE**: The location of
   the Wic kickstart  file used
...
-  **PREFERRED_PROVIDER_virtual/kernel <PREFERRED_PROVIDER>**:
...
-  **PREFERRED_VERSION_linux-yocto <PREFERRED_VERSION>**:
...
-  **UBOOT_* <UBOOT_ENTRYPOINT>**: Defines
...
`/openembedded-core/tree/meta/recipes-kernel/linux`.
