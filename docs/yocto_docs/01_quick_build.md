...
Project, see the /overview-manual/index.

## Compatible Linux Distribution

Make sure your **Build Host** meets the
...
-  Runs a supported Linux distribution (i.e. recent releases of Fedora,
   ...
   ref-manual/system-requirements:supported linux distributions
...
> see the ref-manual/system-requirements:required packages for the build host
...
`$HOME/bitbake-builds`, you can set it with the bitbake:ref-bbsetup-command-settings command:
...
> **Note:**
> Use bitbake-setup settings list 
> to get an overview of the settings.
>

bitbake:ref-bbsetup-command-init is an interactive program by default and
...
#. Choose a **bitbake:setup** directory name:
...
The `init` command creates a new **bitbake:Setup** in the
**bitbake:top directory**. The default name is derived from the selected
configuration above.
...
-  The **bitbake:BitBake Build** directory, named `build`. Later, when the
...
-  The **layers <Layer>** needed to build the Poky reference distribution,
...
> **Note:**
> It is also possible to setup the **Poky** reference distro manually. For
> that refer to the /dev-manual/poky-manual-setup section of the Yocto
...
#.  **Initialize the Build Environment:** Source the `init-build-env`
    environment setup script within the **bitbake:BitBake build** directory
    to setup the **BitBake** build environment on your host:
...
#.  **Examine Your Current Configuration:** When you set up the build
    ...
    sub-directory of the **bitbake:BitBake build** directory. This file is
    ...
    .. note::
       ...
       previously, see the /dev-manual/poky-manual-setup section of the
    ...
    ref-fragments-root-login-with-empty-password fragment:
    ...
    .. note::
       ...
       failures by using overview-manual/concepts:shared state cache
       mirrors and enabling overview-manual/concepts:hash equivalence.
       ...
       install a **Buildtools** tarball by following the instructions in
       the system-requirements-buildtools section of the Yocto Project
       Reference Manual.
...
#. **Start the Build:** Continue with the following command to build an OS
   ...
   overview-manual/concepts:bitbake section in the Yocto Project Overview and
   Concepts Manual, or see
   bitbake-user-manual/bitbake-user-manual-intro:the bitbake command
...
#. **Simulate Your Image Using QEMU:** Once this particular image is
   ...
   dev-manual/qemu:using the quick emulator (qemu) chapter in
...
#.  **Find a Layer:** Many BSP layers are available. The
    `layer index <>` can be used to find such layers. This example
    adds the meta-raspberrypi  BSP
    layer.
...
#.  **Add Your Layer to the Layer Configuration File:** Before you can use
    ...
    file, which is found in the **Build Directory** (`conf/`) directory.
...
#.  **Change the Configuration to Build for a Specific Machine:** The
    **MACHINE** variable is defined by the ref-fragments-builtin-core-machine
    fragment. For this example, the meta-raspberrypi layer provides the
    raspberrypi5 
    machine, so let's make it the **MACHINE** used for the build with
    ...
    Add the `synaptics-killswitch` value to the **LICENSE_FLAGS_ACCEPTED**
...
-  **Website:** The `Yocto Project Website <>` provides
...
-  **Yocto Project Overview and Concepts Manual:** The
   /overview-manual/index is a great
...
-  **Yocto Project Wiki:** The `Yocto Project Wiki <>`
...
-  **Yocto Project Mailing Lists:** Related mailing lists provide a forum
   ...
   ref-manual/resources:mailing lists
